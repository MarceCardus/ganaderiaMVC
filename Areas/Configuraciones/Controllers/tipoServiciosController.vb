Imports System
Imports System.Collections.Generic
Imports System.Data
Imports System.Data.Entity
Imports System.Linq
Imports System.Net
Imports System.Web
Imports System.Web.Mvc
Imports ganaderiaMVC

Namespace Areas.Configuraciones.Controllers
    Public Class tipoServiciosController
        Inherits System.Web.Mvc.Controller

        Private db As New ganaderiaEntities

        ' GET: tipoServicios
        Function Index() As ActionResult
            Return View(db.tipoServicios.ToList())
        End Function

        ' GET: tipoServicios/Details/5
        Function Details(ByVal id As Integer?) As ActionResult
            If IsNothing(id) Then
                Return New HttpStatusCodeResult(HttpStatusCode.BadRequest)
            End If
            Dim tipoServicio As tipoServicio = db.tipoServicios.Find(id)
            If IsNothing(tipoServicio) Then
                Return HttpNotFound()
            End If
            Return View(tipoServicio)
        End Function

        ' GET: tipoServicios/Create
        Function Create() As ActionResult
            Return View()
        End Function

        ' POST: tipoServicios/Create
        'Para protegerse de ataques de publicación excesiva, habilite las propiedades específicas a las que quiere enlazarse. Para obtener 
        'más detalles, vea https://go.microsoft.com/fwlink/?LinkId=317598.
        <HttpPost()>
        <ValidateAntiForgeryToken()>
        Function Create(<Bind(Include:="tsCod,tsNombre")> ByVal tipoServicio As tipoServicio) As ActionResult
            If ModelState.IsValid Then
                db.tipoServicios.Add(tipoServicio)
                db.SaveChanges()
                Return RedirectToAction("Index")
            End If
            Return View(tipoServicio)
        End Function

        ' GET: tipoServicios/Edit/5
        Function Edit(ByVal id As Integer?) As ActionResult
            If IsNothing(id) Then
                Return New HttpStatusCodeResult(HttpStatusCode.BadRequest)
            End If
            Dim tipoServicio As tipoServicio = db.tipoServicios.Find(id)
            If IsNothing(tipoServicio) Then
                Return HttpNotFound()
            End If
            Return View(tipoServicio)
        End Function

        ' POST: tipoServicios/Edit/5
        'Para protegerse de ataques de publicación excesiva, habilite las propiedades específicas a las que quiere enlazarse. Para obtener 
        'más detalles, vea https://go.microsoft.com/fwlink/?LinkId=317598.
        <HttpPost()>
        <ValidateAntiForgeryToken()>
        Function Edit(<Bind(Include:="tsCod,tsNombre")> ByVal tipoServicio As tipoServicio) As ActionResult
            If ModelState.IsValid Then
                db.Entry(tipoServicio).State = EntityState.Modified
                db.SaveChanges()
                Return RedirectToAction("Index")
            End If
            Return View(tipoServicio)
        End Function

        ' GET: tipoServicios/Delete/5
        Function Delete(ByVal id As Integer?) As ActionResult
            If IsNothing(id) Then
                Return New HttpStatusCodeResult(HttpStatusCode.BadRequest)
            End If
            Dim tipoServicio As tipoServicio = db.tipoServicios.Find(id)
            If IsNothing(tipoServicio) Then
                Return HttpNotFound()
            End If
            Return View(tipoServicio)
        End Function

        ' POST: tipoServicios/Delete/5
        <HttpPost()>
        <ActionName("Delete")>
        <ValidateAntiForgeryToken()>
        Function DeleteConfirmed(ByVal id As Integer) As ActionResult
            Dim tipoServicio As tipoServicio = db.tipoServicios.Find(id)
            db.tipoServicios.Remove(tipoServicio)
            db.SaveChanges()
            Return RedirectToAction("Index")
        End Function

        Protected Overrides Sub Dispose(ByVal disposing As Boolean)
            If (disposing) Then
                db.Dispose()
            End If
            MyBase.Dispose(disposing)
        End Sub
    End Class
End Namespace
